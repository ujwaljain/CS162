...
public class CommunicatorGrader1 extends BasicTestGrader
{
	static int count = 0;
	...
	public void run () {
		...
		SpeakerThenListener(1, 2000);
		SpeakerThenListener(3, 2000);
		SpeakerThenListener(10, 3000);
		SpeakerThenListener(20, 8000);
		SpeakerThenListener(50, 30000);
		done();
	}

	/* Forks all the speakers at once then forks listeners */
	private void SpeakerThenListener(int speakers, int time) {
		System.out.println("Forking " + speakers + " Speakers then Listeners");
		count = 0; 
		list.clear();
